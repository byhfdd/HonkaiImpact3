...
public class HeavenGenerate implements IWorldGenerator {
    ...
    public void generate(Random random, int x, int z, World w, IChunkProvider chunkGenerator,
    ...
    public void generateSurface(Random random, int x, int z, World world) {
        addOreSpawn(
            BlockLoader.EtherPhlogistonore01,
            world,
            random,
            x,
            z,
            16,
            16,
            10 + random.nextInt(6 + this.EventSize),
            10 + this.EventSpawn,
            1,
            128,
            Blocks.stone);
        addOreSpawn(
            BlockLoader.EtherPhlogistonore02,
            world,
            random,
            x,
            z,
    ...
    // }
    //
    // public void addOreSpawn(Block block, World world, Random random, int x, int z, int maxX, int maxZ, int Size, int
    // Spawn, int minY, int maxY) {
    // int diffBtwnMinMaxY = maxY - minY;
    ...
    // for (int s = 0; s < 10; ++s) {
    // this.addOreSpawn(BlockLoader.EtherPhlogistonore02, world, random, x, z, 16, 16, 5 + random.nextInt(6), 6, 1, 128,
    // ItemCraft10.SkyStone);
    // }
    // }
    // }
    //
    // public void addOreSpawn(Block block, World world, Random random, int blockXPos, int blockZPos, int maxX, int
    // maxZ, int maxVeinSize, int chancesToSpawn, int minY, int maxY, Block replaceblock) {
    // int diffBtwnMinMaxY = maxY - minY;
    //
    // for (int x = 0; x < chancesToSpawn; ++x) {
    // int posX = blockXPos + random.nextInt(maxX);
